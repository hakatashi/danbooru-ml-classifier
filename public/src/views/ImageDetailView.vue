    ...
    <div v-else-if="image" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div class="space-y-4">
        ...
        <div class="bg-white rounded-xl shadow-md p-4">
          <h2 class="text-lg font-semibold text-gray-900 mb-4">Image Information</h2>
          <dl class="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <dd class="font-mono text-gray-900">{{ image.postId || 'N/A' }}</dd>
            </div>
            <div>
              <dt class="text-gray-500">Status</dt>
              <dd class="text-gray-900 capitalize">{{ image.status || 'unknown' }}</dd>
            </div>
            <div class="col-span-2">
              <dt class="text-gray-500">Document ID</dt>
              <dd class="font-mono text-xs text-gray-900 break-all">{{ image.id }}</dd>
            </div>
          </dl>
        </div>
        ...
        <!-- Moderation Ratings -->
        <div class="bg-white rounded-xl shadow-md p-4">
          <h2 class="text-lg font-semibold text-gray-900 mb-4">Moderation Ratings</h2>
          <div class="space-y-3">
            <div v-for="model in models" :key="model" class="flex items-center justify-between">
              <span class="font-medium text-gray-700 capitalize">{{ model }}</span>
      ...
      <!-- Right: Captions -->
      <div class="space-y-6">
        <div v-for="model in models" :key="model" class="bg-white rounded-xl shadow-md overflow-hidden">
          <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
            <div class="flex items-center justify-between">
              <h3 class="font-semibold text-gray-900 capitalize">{{ model }}</h3>
              <div class="flex items-center gap-2">
                <span
                  :class="[
                    getRatingColorClass(image.moderations?.[model]?.result ?? null),
          ...
          <div class="p-4">
            ...
            <div class="mt-3 flex items-center gap-2 flex-wrap">
              ...
              >
                <span class="w-4 h-4 flex items-center justify-center bg-blue-600 text-white rounded text-[10px] font-bold">
                  {{ translator.icon }}
                </span>
                {{ translator.name }}
              </a>
          ...
          <div v-if="image.moderations?.[model]?.raw_result" class="px-4 pb-4">
            <details class="group">
              ...
              <div class="mt-2 bg-gray-50 rounded-lg p-3 text-xs font-mono text-gray-600 whitespace-pre-wrap">
